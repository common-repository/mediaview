...
class mvMediaItemListTable extends mvWPListTableBase{
	...
	function get_sortable_columns() 
	{
		...
		$sortable_columns = array(
			'mv-itemtype'  => array('mv-itemtype', false),
	...
	function process_bulk_action()
	{
		...
		if($action != -1){
			...
			if($action == 'delete')
				mvAdminGen::deleteMediaItems($_POST['mediaitem'], $wpdb);
	...
	function setup_items()
	{
		...
		foreach($data as $val)
		{
			...
			array_push($cells, array(
				'ID' => $val['MEDIA_ID'],
				'mv-itemthumbnail' => '<img src="' . $dir . '/mediaview-cache/' . $data['thumb'] . '" class="mv-prev-thumb"/><span class="mv-sortable-handle">::</span>',
				...
				'dateadd' => date('m/d/Y', $val['MEDIA_UPDATEDATE'])
			));
